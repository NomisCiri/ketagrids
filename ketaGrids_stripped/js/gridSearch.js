...
var totalTrialsNumber = 10, // includes the instructions trial and the bonus trial
...
function onCellTapped(cell) {
  ...
  if (clicks == 0) {
    ...
    var remainingMsg = (trials) == 1 ? "weiteres Feld." : "weitere Felder.";
  ...
  if (trials == 0 && clicks == 0) {
      ...
      completionDiv = "<h1 class=\"text-xl\">Wow! Du hast insgesamt ganze <b>" + finalStarCount + " Sterne gesammelt</b>! Vielen Dank fürs Spielen!</b> </h1>" + finalStarDiv + "";
...
function senddata() { 
  ...
  saveText(JSON.stringify(saveDataArray), 'gridSearch.' +participantId+'.'+session+ '.JSON');
